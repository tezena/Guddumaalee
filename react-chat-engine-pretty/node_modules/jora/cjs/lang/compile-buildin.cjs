...
function bool(value) {
    ...
    if (utils.isPlainObject(value)) {
        for (const key in value) {
            if (utils.hasOwnProperty.call(value, key)) {
                return true;
            }
        }
...
function in_(a, b) {
    if (utils.isPlainObject(b)) {
        return utils.hasOwnProperty.call(b, a);
    }
...
function pick(current, ref = () => true) {
    ...
    if (typeof ref === 'function') {
        ...
        for (const key in current) {
            if (utils.hasOwnProperty.call(current, key)) {
    ...
    return utils.hasOwnProperty.call(current, ref) ? current[ref] : undefined;
}

function map(value, getter) {
    const fn = typeof getter === 'function'
        ? getter
        : current => utils.getPropertyValue(current, getter);

    if (Array.isArray(value)) {
        return [
            ...value.reduce(
                (set, item) => utils.addToSet(set, fn(item)),
                new Set()
            )
        ];
    }
...
function mapRecursive(value, getter) {
    ...
    utils.addToSet(result, map(value, getter));

    result.forEach(current =>
        utils.addToSet(result, map(current, getter))
    );
