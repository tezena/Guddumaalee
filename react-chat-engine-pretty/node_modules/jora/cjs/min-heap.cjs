...
class MaxHeap {
    constructor(maxSize, accept, comparator) {
        ...
        this.values = [];
        this.passAccepted = new Set();
        this.enter = 0;
    }

    add(value) {
        this.enter++;
        if (this.passAccepted.has(value)) {
            return;
        }
    ...
    heapify(idx) {
        ...
        // with smallestIdx to fix the min-heap property and recursively call heapify for node smallestIdx
        if (smallestIdx !== idx) {
            // swap
    ...
    [Symbol.iterator]() {
        console.log({ count: this.enter, passAccepted: this.passAccepted.size });
