...
function createQuery(source, options) {
    ...
    fn = fn(compileBuildin, localMethods);

    return statMode
        ? (data, context) => stat(source, fn(data, context))
        : fn;
}
...
function setup(customMethods) {
    ...
    for (const [name, fn] of Object.entries(customMethods || {})) {
        if (typeof fn === 'string') {
            Object.defineProperty(localMethods, name, {
                configurable: true,
                get() {
                    const compiledFn = compileFunction(fn)(compileBuildin, localMethods);
    ...
    return function query(source, options) {
        ...
        if (cache.has(source) && !options.debug) {
            ...
            const perform = compileFunction(source, statMode, tolerantMode, options.debug)(compileBuildin, localMethods);
            fn = statMode
                ? (data, context) => stat(source, perform(data, context))
                : perform;
            cache.set(source, fn);
        }
